...
var User = new EntitySchema({
  name: 'User',
  tableName: 'user',
  // Optional: specify table name if different from entity name
  columns: {
    id: {
      primary: true,
      type: 'int',
  ...
  relations: {
    requests: {
      target: 'Request',
      // Reference the Request entity by its name
      type: 'one-to-many',
      inverseSide: 'user',
      // Property on the Request entity that refers back to User
